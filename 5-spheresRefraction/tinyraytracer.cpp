...
struct Material {
    ...
    Material() : refractive_index(), albedo(), diffuse_color(), specular_exponent() {}
...
vec3 refract(const vec3 &I, const vec3 &N, const float &refractive_index) { // Snell's law
...
void render(const std::vector<Sphere> &spheres, const std::vector<Light> &lights)
{
    ...
    ofs.open("./outSpheresRefractionImage.ppm", std::ios::binary);
    ofs << "P6\n" << width << " " << height << "\n255\n";
...
int main() {
    Material purpel_material(1.0, vec4{0.6,  0.3, 0.1, 0.0}, vec3{0.58, 0.44, 0.86}, 50);
    Material red_material(1.0, vec4{0.9,  0.1, 0.0, 0.0}, vec3{1.0, 0.42, 0.42}, 10);
